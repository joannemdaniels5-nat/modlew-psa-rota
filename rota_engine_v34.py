...
def schedule_week(tpl: TemplateData, wk_start: date):
    ...
    for d in dates:
        for idx, t in enumerate(slots):
            ...
            # Mandatory: Front Desk — 1 per site per slot (highest priority)
            for site in SITES:
                role = f"FrontDesk_{site}"
                current = [nm for nm in staff_names if a.get((d,t,nm)) == role]
                if len(current) >= 1:
                    # If duplicates somehow occur, keep best and free the rest
                    if len(current) > 1:
                        def fd_score(nm: str):
                            st = staff_by_name[nm]
                        ...
                        for extra in current[1:]:
                            ...
                            add_mins(d, extra, "Misc", SLOT_MIN)
                            gaps.append((d, t, role, f"Removed duplicate FD for {extra}; kept {keep}"))
                    continue
