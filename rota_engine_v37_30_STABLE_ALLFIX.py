...
class TemplateData:
    ...
    swaps: List[object]  # legacy tuples or SwapOverride entries
...
# --- Holidays ranges
    ...
    # Swaps / Overrides
    # Supported formats:
    #  A) Legacy: Date, Name, SwapWith, NewStart, NewEnd  (swap shifts or change times)
    #  B) Clean overrides: Name, Date, Start, End, InOut (IN/OUT)
    #     - Use OUT to remove working time (appointments etc.)
    #     - Use IN to add working time (extra hours or swapping in on a non-working day)
    swaps: List[object] = []
    if swaps_df is not None and not swaps_df.empty:
        dcol = pick_col(swaps_df, ['Date'], required=False) or swaps_df.columns[0]
        ncol = pick_col(swaps_df, ['Name','StaffName'], required=False) or swaps_df.columns[1]

        # New format fields
        actcol = pick_col(swaps_df, ['InOut','Action','IN/OUT','In Out'], required=False)
        stcol2 = pick_col(swaps_df, ['Start','StartTime','From'], required=False)
        encol2 = pick_col(swaps_df, ['End','EndTime','To','Finish'], required=False)

        # Legacy fields
        swcol = pick_col(swaps_df, ['SwapWith','Swap With'], required=False)
        nscol = pick_col(swaps_df, ['NewStart','New Start'], required=False)
        necol = pick_col(swaps_df, ['NewEnd','New End'], required=False)
        ...
        for _, r in swaps_df.iterrows():
            ...
            # New clean override format (preferred)
            if actcol:
                act = str(r.get(actcol,'')).strip().upper()
                if act in {'IN','OUT'}:
                    st = to_time(r.get(stcol2)) if stcol2 else None
                    en = to_time(r.get(encol2)) if encol2 else None
                    swaps.append(SwapOverride(date=dd, name=nm, action=act, start=st, end=en))
                    continue

            # Legacy format (swap / change times)
...
def shift_window_effective(hours_map: Dict[str, Dict[str, Optional[time]]], d: date, name: str, overrides=None) -> Tuple[Optional[time], Optional[time]]:
    stt, end = shift_window(hours_map, d, name)
    if not overrides:
        return stt, end
    ins = overrides.get("IN", {}).get((d, name), [])
    if ins:
        starts = [s for s, e in ins if s and e]
        ends = [e for s, e in ins if s and e]
        if starts and ends:
            if stt is None or end is None:
                stt, end = min(starts), max(ends)
            else:
                stt, end = min([stt] + starts), max([end] + ends)
    return stt, end

def is_working_effective(hours_map: Dict[str, Dict[str, Optional[time]]], d: date, t: time, name: str, overrides=None) -> bool:
    base_st, base_end = shift_window(hours_map, d, name)
    in_base = bool(base_st and base_end and (t >= base_st) and (t < base_end))

    in_extra = False
    if overrides:
        for s, e in overrides.get("IN", {}).get((d, name), []):
            if s is None and e is None:
                in_extra = True
                break
            if s and e and (t >= s) and (t < e):
                in_extra = True
                break

    if not (in_base or in_extra):
        return False

    if overrides:
        for s, e in overrides.get("OUT", {}).get((d, name), []):
            if s is None and e is None:
                return False
            if s and e and (t >= s) and (t < e):
                return False
...
def apply_swaps(hours_map: Dict[str, Dict[str, Optional[time]]], swaps, week_dates: List[date]):
    """Apply swap overrides (whole-day or partial-day)."""
    out = {k: dict(v) for k, v in hours_map.items()}
    in_week = set(week_dates)
    overrides = {"IN": {}, "OUT": {}}

    def add_iv(kind: str, dd: date, nm: str, st: Optional[time], en: Optional[time]):
        if dd not in in_week:
            return
        overrides[kind].setdefault((dd, nm), []).append((st, en))

    for item in swaps:
        if isinstance(item, SwapOverride):
            if item.date in in_week:
                add_iv(item.action, item.date, item.name, item.start, item.end)
            continue
...
def pick_breaks_site_balanced(staff_list: List[Staff], hours_map, hols, week_dates: List[date], fixed_assignments: Set[Tuple[date,time,str]]) -> Dict[Tuple[date,time], Set[str]]:
    ...
    for d in week_dates:
        for st in staff_list:
            ...
            # If someone starts at/after 11:00 and needs a break, avoid breaks too early.
            cand_times = BREAK_CANDIDATES
            if stt >= time(11, 0):
                cand_times = [time(14, 0), time(14, 30), time(15, 0), time(15, 30)]

            for bt in cand_times:
...
def schedule_week(tpl: TemplateData, wk_start: date):
    ...
    # apply swaps
    hours_map, swap_overrides = apply_swaps(tpl.hours_map, tpl.swaps, dates)
    ...
    def can_cover_full_band(nm: str, d: date, bs: time, be: time) -> bool:
        ...
        stt, endt = shift_window_effective(hours_map, d, nm, swap_overrides)
...
# --- Breaks (only >6h and BreakRequired)
    ...
    def eligible(nm: str, task: str, d: date, t: time, allow_cross_site: bool=False) -> bool:
        ...
        if holiday_kind(nm, d, tpl.hols):
            return False
        if not is_working_effective(hours_map, d, t, nm, swap_overrides):
            return False
        if on_break(nm, d, t):
            return False
    ...
    def start_block(nm: str, task: str, d: date, start_idx: int, allow_short_end: bool=True) -> bool:
        ...
        stt, end = shift_window_effective(hours_map, d, nm, swap_overrides)
        if not stt or not end:
            return False
    ...
    return a, breaks, gaps, dates, slots, hours_map, swap_overrides, swap_overrides
# ---------- Excel output ----------

def _norm_argb(hex_color: str) -> str:
    """Ensure Excel-safe ARGB (8 hex). Accepts 'RRGGBB' or 'AARRGGBB'."""
    if hex_color is None:
        return "FFFFFFFF"
    s = str(hex_color).strip().replace("#","").upper()
...
def fill_for(value: str) -> PatternFill:
    return PatternFill("solid", fgColor=_norm_argb(ROLE_COLORS.get(value, "FFFFFF")))
...
def build_workbook(tpl: TemplateData, start_monday: date, weeks: int) -> Workbook:
    ...
    COVER_TASKS = [
        ...
        "Triage_Admin_SLGP","Triage_Admin_JEN","Triage_Admin_BGS","Triage_Admin_BGS","Triage_Admin_BGS",
    ...
    for w in range(weeks):
        ...
        a, breaks, gaps, dates, slots, hours_map, swap_overrides = schedule_week(tpl, wk_start)
        ...
        for d in dates:
            for t in slots:
                ...
                for nm in staff_names:
                    ...
                    if hk:
                        val = hk
                    elif not is_working_effective(hours_map, d, t, nm, swap_overrides):
                        val = ""
        ...
        tasks_tot = [
            "FrontDesk_SLGP","FrontDesk_JEN","FrontDesk_BGS",
            "Triage_Admin_SLGP","Triage_Admin_JEN","Triage_Admin_BGS","Triage_Admin_BGS","Triage_Admin_BGS","Email_Box",
        ...
        ws_dash.conditional_formatting.add(f"D3:D{last_row}", DataBarRule(start_type="num", start_value=0, end_type="num", end_value=1, color="FF63C384"))
        ...
        for d in dates:
            for t in slots:
                ...
                row.append(names_for_prefix("Triage_Admin_BGS", d, t))
    ...
    # ---------------- Swaps sheet (for optional IN/OUT overrides) ----------------
    # Keeping this in the output workbook makes it easy for managers to record changes for next run.
    if "Swaps" not in wb.sheetnames:
        ws_sw = wb.create_sheet("Swaps")
        ws_sw.append(["Name","Date","Start","End","InOut"])
        for c in ws_sw[1]:
            c.font = Font(bold=True)
            c.alignment = Alignment(horizontal="center", vertical="center")
        ws_sw.column_dimensions["A"].width = 22
        ws_sw.column_dimensions["B"].width = 14
        ws_sw.column_dimensions["C"].width = 10
        ws_sw.column_dimensions["D"].width = 10
        ws_sw.column_dimensions["E"].width = 10
        # Populate any swaps from the input template (both legacy + new)
        try:
            for item in getattr(tpl, "swaps", []) or []:
                if isinstance(item, SwapOverride):
                    ws_sw.append([item.name, item.date, item.start, item.end, item.action])
                else:
                    dd, nm, sw, ns, ne = item
                    ws_sw.append([nm, dd, ns, ne, "IN"])
        except Exception:
            pass
    return wb
...
def _task_color(task: str) -> str:
    return _norm_argb(ROLE_COLORS.get(task, "FFFFFF"))

def _apply_fill_and_style(ws, start_row, start_col, end_row, end_col, repeat_header_every=None, header_row=1):
    # Apply colors/borders to a rectangular region.
    # For timeline sheets we want: row height 35, full cell borders, thick border around each day block.
    for r in range(start_row, end_row + 1):
        # skip repeated headers
        if repeat_header_every and ((r - start_row) % repeat_header_every == 0) and r != start_row:
        ...
        bottom_day = (time_val == "18:00")

        ws.row_dimensions[r].height = 35.0
        ...
        for c in range(start_col, end_col + 1):
            ...
            cell.border = Border(
                ...
                bottom=THICK_SIDE if bottom_day else THIN_SIDE,
                left=THIN_SIDE,
                right=THIN_SIDE,
            )
    ...
    for ws in site_sheets.values():
        ...
        for r in range(2, ws.max_row + 1):
            ...
            for nm in staff:
                ...
                if v not in {"Break", "Holiday", "Sick", "Bank Holiday"}:
...
def _write_coverage_sheet_from_site_timelines(wb, week_num: int):
    ...
    for rr in range(2, ws_cov.max_row+1):
        ...
        top_day = (time_val == DAY_START.strftime("%H:%M"))
        ws_cov.row_dimensions[rr].height = 35.0
        for cc in range(1, ws_cov.max_column+1):
            cell = ws_cov.cell(rr,cc)
            cell.alignment = Alignment(vertical="top", wrap_text=True)
            cell.border = Border(top=THICK_SIDE if top_day else THIN_SIDE, bottom=THICK_SIDE if str(ws_cov.cell(rr,2).value or "")=="18:00" else THIN_SIDE, left=THIN_SIDE, right=THIN_SIDE)
...
def _write_targets_heatmap_sheet(tpl: TemplateData, wb, week_num: int, assignments, dates, slots, staff_names):
    """Create a simple heatmap-style sheet showing Phones (required vs actual) and Bookings (target vs actual) per slot."""
    from openpyxl.styles import Font, Alignment
    from openpyxl.utils import get_column_letter
    from openpyxl.formatting.rule import ColorScaleRule

    title = f"Week{week_num}_Targets_Heatmap"
    if title in wb.sheetnames:
        del wb[title]
    ws = wb.create_sheet(title)

    headers = ["Date", "Time",
               "Phones_Required", "Phones_Actual", "Phones_Coverage",
               "Bookings_Target", "Bookings_Actual", "Bookings_Coverage"]
    ws.append(headers)
    for c in ws[1]:
        c.font = Font(bold=True)
        c.alignment = Alignment(horizontal="center", vertical="center")
    ...
    for d in dates:
        ...
        for t in slots:
            # Phones
            p_req = int(phones_required(tpl, d, t) or 0)
            p_act = sum(1 for nm in staff_names if assignments.get((d, t, nm)) == "Phones")
            p_cov = (p_act / p_req) if p_req > 0 else 1.0

            # Bookings hourly target (same target applies to both half-hour slots within that hour)
            b_tgt = int(tpl.bookings_targets.get((dn, time(t.hour, 0)), 0) or 0)
            b_act = sum(1 for nm in staff_names if assignments.get((d, t, nm)) == "Bookings")
            b_cov = (b_act / b_tgt) if b_tgt > 0 else 1.0

            ws.append([d.strftime("%a %d-%b"), t.strftime("%H:%M"),
                       p_req, p_act, round(p_cov, 3),
                       b_tgt, b_act, round(b_cov, 3)])

    # Heatmap on coverage columns (E and H)
    last = ws.max_row
    ...
    ws.conditional_formatting.add(f"E2:E{last}", rule)
...
def _rebuild_heatmap_actuals_from_timelines(wb, week_num: int):
    """After users edit site timelines, update the Heatmap sheet's HAVE/GAP columns (targets remain unchanged)."""
    heat_name = f"Week{week_num}_Targets_Heatmap"
    if heat_name not in wb.sheetnames:
        return

    # Need the site timelines
    site_sheets = {site: wb[f"Week{week_num}_{site}_Timeline"] for site in ("SLGP","JEN","BGS") if f"Week{week_num}_{site}_Timeline" in wb.sheetnames}
    if not site_sheets:
        return
    ...
    # Heatmap layout is: Date | Time | Phones_Target | Phones_Have | Phones_Gap | Bookings_Target | Bookings_Have | Bookings_Gap
    # We'll rebuild Phones_Have/Gap and Bookings_Have/Gap by counting entries on timelines per slot.
    # Build staff headers per site for counting.
    headers = {}
    for site, ws in site_sheets.items():
        headers[site] = [str(ws.cell(1,c).value).strip() for c in range(3, ws.max_column+1) if ws.cell(1,c).value]

    # Build a quick row lookup on each site timeline: (date_str, time_str) -> row index
    row_map = {}
    for site, ws in site_sheets.items():
        m = {}
        for r in range(2, ws.max_row+1):
            if str(ws.cell(r,2).value or '').strip().lower() == 'time':
                continue
            dval = str(ws.cell(r,1).value or '').strip()
            tval = str(ws.cell(r,2).value or '').strip()
            if dval and tval:
                m[(dval, tval)] = r
        row_map[site] = m

    for r in range(2, ws_h.max_row+1):
        dval = str(ws_h.cell(r,1).value or '').strip()
        tval = str(ws_h.cell(r,2).value or '').strip()
        if not dval or not tval:
            continue

        phones_have = 0
        books_have = 0
        for site, ws in site_sheets.items():
            rr = row_map[site].get((dval, tval))
            if not rr:
                continue
            for ci, _nm in enumerate(headers[site], start=3):
                v = str(ws.cell(rr,ci).value or '').strip()
                if v == "Phones":
                    phones_have += 1
                elif v == "Bookings":
                    books_have += 1
        ...
        # Phones
        ph_t = ws_h.cell(r,3).value
        try:
            ph_t = int(float(ph_t)) if ph_t not in (None,"") else 0
        except Exception:
            ph_t = 0
        ws_h.cell(r,4).value = phones_have
        ws_h.cell(r,5).value = max(0, ph_t - phones_have)

        # Bookings
        bk_t = ws_h.cell(r,6).value
        try:
            bk_t = int(float(bk_t)) if bk_t not in (None,"") else 0
        except Exception:
            bk_t = 0
        ws_h.cell(r,7).value = books_have
        ws_h.cell(r,8).value = max(0, bk_t - books_have)
...
def recalc_workbook_from_site_timelines(xlsx_bytes: bytes) -> bytes:
    ...
    for w in week_nums:
        ...
        # Rebuild heatmap after recalculation
        try:
            _rebuild_heatmap_actuals_from_timelines(wb, w)
        except Exception:
            pass
...
def build_workbook(tpl: TemplateData, start_monday: date, weeks: int) -> Workbook:
    ...
    for w in range(weeks):
        ...
        assignments, breaks, gaps, dates, slots, hours_map, swap_overrides = schedule_week(tpl, week_start)
        ...
        # Targets heatmap (Phones + Bookings)
        _write_targets_heatmap_sheet(tpl, wb, w+1, assignments, dates, slots, staff_names)
...
def schedule_week_v37_9(tpl: TemplateData, wk_start: date):
    ...
    for d in dates:
        for nm in staff_names:
            ...
            # Candidate break starts
            candidates = []
            for bt in BREAK_CANDIDATES:
                if not (bt >= stt and add_minutes(bt, 30) <= endt):
                    continue
                # Must be within the configured break window
                if not t_in_range(bt, BREAK_WINDOW[0], BREAK_WINDOW[1]):
                    continue
                # Never place a break within the first 2 hours of a shift
                if dt_of(d, bt) < (dt_of(d, stt) + timedelta(hours=2)):
                    continue
                # If someone starts at/after 11:00, only consider 14:00–16:00
                if stt >= time(11, 0):
                    if bt < time(14, 0) or bt > time(15, 30):
                        continue
    ...
    return a, breaks, gaps, dates, slots, hours_map, swap_overrides
