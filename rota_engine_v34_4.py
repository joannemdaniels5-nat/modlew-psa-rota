...
def email_site_for_day(d: date) -> str:
    # Email rota pattern (hard preference; cross-site fallback allowed in scheduler):
    # Mon = BGS, Tue/Wed/Thu = JEN, Fri = SLGP
    wd = d.weekday()
    if wd == 0:  # Mon
        return "BGS"
    if wd in (1, 2, 3):  # Tue/Wed/Thu
        return "JEN"
    return "SLGP"  # Fri (and default)
        ...
        for idx, t in enumerate(slots):
            ...
            for nm in staff_names:
                ...
                if on_break(nm, d, t):
                    continue
                apply_active(nm, d, idx)
            # ---------------- Email coverage (10:30–16:00) ----------------
            # Must be covered by exactly 1 eligible staff member per slot.
            ...
            if t_in_range(t, time(10,30), time(16,0)):
                ...
                if len(current) == 1:
                    ...
                    # Prefer continuity if still valid
                    chosen = None
                    if email_last and can_do_email(email_last) and is_free(email_last, d, t):
                        chosen = email_last
                    else:
                        # Prefer free candidates on preferred site
                        ...
                        # If none free, allow "steal" from non-fixed tasks (never steal FD/Triage)
                        if not chosen:
                            steal_cands = [
