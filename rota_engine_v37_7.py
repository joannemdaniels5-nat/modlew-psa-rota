...
def schedule_week(tpl: TemplateData, wk_start: date):
    ...
    def assign_frontdesk_for_site_day(site: str, d: date) -> None:
        ...
        if picked is None:
            # log gaps for each band start time
    ...
    for d in dates:
        ...
        for idx, t in enumerate(slots):
            ...
            # Email MUST be covered. Prefer the site-of-day, but allocate cross-site if needed.
            # Keep continuity: aim for one person for the full window; if impossible, use the minimum number of handovers.
            # No cover required during the email person's break.
            if t == time(10,30):
                ...
                for tt in window_slots:
                    # continue current person if still eligible
                    if current_email:
                        st = staff_by_name[current_email]
                        if (
                            st.can_email
                            and (not getattr(st, "frontdesk_only", False))
                            and (not holiday_kind(current_email, d, tpl.hols))
                            and is_working(hours_map, d, tt, current_email)
                            and (current_email not in breaks.get((d, tt), set()))
                            and (d, tt, current_email) not in fixed_slots
                        ):
                            a[(d, tt, current_email)] = "Email_Box"
                            fixed_slots.add((d, tt, current_email))
                            add_mins(d, current_email, "Email", SLOT_MIN)
                            continue
                        else:
                            current_email = None

                    # choose a new email owner for this slot
                    candidates = []
                    for nm in staff_names:
                        st = staff_by_name[nm]
                        if not st.can_email:
                    ...
                    if not candidates:
                        gaps.append((d, tt, "Email_Box", "No suitable staff for Email slot"))
                        continue
