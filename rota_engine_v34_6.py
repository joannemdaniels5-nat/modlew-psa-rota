...
# =========================
# OVERRIDES / PRODUCTION FIXES (v34.6+) 
# - No blank working slots on site timelines (even if task is 'site-restricted')
# - Swaps sheet supports IN/OUT overrides
# - Recalc recolours site timelines + coverage
# - Holiday colour distinct from Front Desk
# =========================

# Make Holiday visibly different from Front Desk
try:
    ROLE_COLORS["Holiday"] = "E2F0D9"  # light mint
except Exception:
    pass

# --- Swaps parsing (IN/OUT) -----------------------------------
_read_template_original = read_template

def read_template(uploaded_bytes: bytes) -> TemplateData:
    tpl = _read_template_original(uploaded_bytes)

    # Re-parse swaps sheet in the new simple format (if present)
    try:
        xls = pd.ExcelFile(io.BytesIO(uploaded_bytes))
        swaps_sheet = find_sheet(xls, ["Swaps", "Swap"])
        if not swaps_sheet:
            return tpl
        swaps_df = pd.read_excel(xls, sheet_name=swaps_sheet)
        if swaps_df is None or swaps_df.empty:
            return tpl

        dcol = pick_col(swaps_df, ["Date"], required=False) or swaps_df.columns[0]
        ncol = pick_col(swaps_df, ["Name","StaffName"], required=False) or swaps_df.columns[1]
        dircol = pick_col(swaps_df, ["InOut","In/Out","Direction"], required=False)
        stcol = pick_col(swaps_df, ["StartTime","Start","NewStart","From"], required=False)
        encol = pick_col(swaps_df, ["FinishTime","Finish","NewEnd","To","End"], required=False)

        swaps_new: List[Tuple[date, str, str, Optional[time], Optional[time]]] = []

        for _, r in swaps_df.iterrows():
            # Gracefully skip guidance rows / blanks
            try:
                dd = to_date(r.get(dcol))
            except Exception:
                dd = None
            ...
            direction = str(r.get(dircol,"IN")).strip().upper() if dircol else "IN"
            if direction not in {"IN","OUT"}:
                continue

            if direction == "OUT":
                swaps_new.append((dd, nm, "OUT", None, None))
                continue

            ns = to_time(r.get(stcol)) if stcol else None
            ne = to_time(r.get(encol)) if encol else None
            if ns and ne:
                swaps_new.append((dd, nm, "IN", ns, ne))

        return TemplateData(
            staff=tpl.staff,
            hours_map=tpl.hours_map,
            hols=tpl.hols,
            call_handlers=tpl.call_handlers,
            handler_leave=tpl.handler_leave,
            phones_targets=tpl.phones_targets,
            bookings_targets=tpl.bookings_targets,
            weekly_targets=tpl.weekly_targets,
            swaps=swaps_new,
            buddies=tpl.buddies,
        )
    except Exception:
...
# --- Apply swaps (IN/OUT + legacy) -----------------------------
def apply_swaps(hours_map: Dict[str, Dict[str, Optional[time]]], swaps, week_dates: List[date]) -> Dict[str, Dict[str, Optional[time]]]:
    out = {k: dict(v) for k, v in hours_map.items()}
    in_week = set(week_dates)

    for dd, nm, mode, ns, ne in swaps:
        if dd not in in_week:
            continue
        dn = day_name(dd)
        if nm not in out:
        ...
        if mode_u == "OUT":
            out[nm][f"{dn}Start"] = None
            out[nm][f"{dn}End"] = None
            continue
        if mode_u == "IN":
            out[nm][f"{dn}Start"] = ns
            out[nm][f"{dn}End"] = ne
            continue

        # legacy swap-with name
        sw = str(mode).strip()
        if sw and sw in out:
            a1, a2 = out[nm].get(f"{dn}Start"), out[nm].get(f"{dn}End")
            b1, b2 = out[sw].get(f"{dn}Start"), out[sw].get(f"{dn}End")
...
# --- Workbook builder (stable, no blanks on site sheets) --------
def build_workbook(tpl: TemplateData, start_monday: date, weeks: int) -> Workbook:
    """Stable build: site timelines are VALUES + FILLS.
    IMPORTANT: we DO NOT hide 'Bookings/Email/Awaiting' if allocated cross-site, so no blank working slots.
    """
    wb = Workbook()
    wb.remove(wb.active)
    ...
    for w in range(weeks):
        ...
        # Site timelines (home site staff only)
        for site in ("SLGP","JEN","BGS"):
            site_staff = [nm for nm in staff_names if str(staff_by_name[nm].home).upper() == site]
            ws = wb.create_sheet(f"Week{w+1}_{site}_Timeline")
            ws.append(["Date","Time"] + site_staff)
            ...
            # Apply fills + borders
            for rr in range(2, ws.max_row+1):
                if str(ws.cell(rr,2).value or '').strip().lower() == 'time':
                    continue
                time_val = str(ws.cell(rr,2).value or "")
                top_day = (time_val == DAY_START.strftime("%H:%M"))
                for cc in range(1, ws.max_column+1):
                    cell = ws.cell(rr,cc)
                    cell.alignment = Alignment(vertical="top", wrap_text=True)
                    if cc >= 3:
                        cell.fill = fill_for(str(cell.value or ""))
                    cell.border = Border(top=THICK_SIDE if top_day else THIN_SIDE, bottom=THIN_SIDE, left=THIN_SIDE, right=THIN_SIDE)

        # Coverage sheet (static)
        tasks_cols = [
            "FrontDesk_SLGP","FrontDesk_JEN","FrontDesk_BGS",
            "Triage_Admin_SLGP","Triage_Admin_JEN",
            "Email_Box","Awaiting_PSA_Admin","Phones","Bookings","EMIS","Docman","Misc_Tasks"
        ...
        # Totals (static from assignments + breaks)
        totals = _compute_totals_from_assignments(assignments, breaks, dates, slots, staff_names)
        ws_tot = wb.create_sheet(f"Week{w+1}_Totals")
        tasks_order = [
            "FrontDesk_SLGP","FrontDesk_JEN","FrontDesk_BGS",
...
# --- Recalc workbook: recolour + rebuild coverage/totals --------
def recalc_workbook_from_site_timelines(xlsx_bytes: bytes) -> bytes:
    wb = load_workbook(io.BytesIO(xlsx_bytes))
    week_nums = sorted({int(m.group(1)) for s in wb.sheetnames if (m:=re.match(r"Week(\d+)_SLGP_Timeline", s))})
    if not week_nums:
    ...
    for w in week_nums:
        ...
        # recolour site timelines after edits
        for site in ("SLGP","JEN","BGS"):
            title = f"Week{w}_{site}_Timeline"
