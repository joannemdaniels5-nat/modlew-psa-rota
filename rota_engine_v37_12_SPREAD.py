...
# Awaiting/PSA Admin is allocated as a stable daily block (see start of day).
            ...
            # --- Spread EMIS/Docman across the week (avoid front-loading everything into Mon/Tue)
            # Per-day caps: aim for an even daily share; allow some flex, but stop one day swallowing the whole weekly target.
            DAY_SPREAD_LOW = 0.80
            DAY_SPREAD_HIGH = 1.30
            day_target_emis = (target_emis / 5.0) if target_emis > 0 else 0.0
            day_target_doc  = (target_doc  / 5.0) if target_doc  > 0 else 0.0
            day_low_emis = int(round(day_target_emis * DAY_SPREAD_LOW)) if day_target_emis else 0
            day_high_emis = int(round(day_target_emis * DAY_SPREAD_HIGH)) if day_target_emis else 0
            day_low_doc = int(round(day_target_doc * DAY_SPREAD_LOW)) if day_target_doc else 0
            day_high_doc = int(round(day_target_doc * DAY_SPREAD_HIGH)) if day_target_doc else 0
            ...
            for _tt in slots:
                if _tt > t:
                    break
                for _nm in staff_names:
                    _v = a.get((d, _tt, _nm))
                    if _v == "EMIS":
                        emis_day_done += SLOT_MIN
                    elif _v == "Docman":
                        doc_day_done += SLOT_MIN
                    elif _v == "Bookings":
                        book_day_done += SLOT_MIN
            ...
            if target_doc > 0 and doc_done < low_doc and (day_high_doc == 0 or doc_day_done < day_high_doc):
                filler_tasks.append("Docman")
            if target_emis > 0 and emis_done < low_emis and (day_high_emis == 0 or emis_day_done < day_high_emis):
            ...
            for nm in staff_names:
                ...
                if chosen in ("EMIS","Docman") and staff_by_name[nm].home == "SLGP":
                    if t >= time(10,30) and target_book > 0 and book_done < low_book and eligible(nm, "Bookings", d, t, allow_cross_site=True):
                        ...
                        # look for any other free JEN/BGS candidate for the same task
                        any_other = False
