...
st.set_page_config(page_title="Rota Generator v16", layout="wide")

st.title("Rota Generator v16 — Weighted + Block-Stable (No Floaters)")
...
if uploaded:
    try:
        ...
        if st.button("Generate rota and download Excel", type="primary"):
            ...
            wb.save(bio)
            bio.seek(0)
            out_name = f"rota_v16_{start_monday.isoformat()}_{weeks}w.xlsx"
            st.download_button(
                "📊 Download Excel rota",
                data=bio.getvalue(),
                file_name=out_name,
