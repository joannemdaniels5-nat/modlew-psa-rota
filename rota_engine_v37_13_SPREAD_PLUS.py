...
def schedule_week(tpl: TemplateData, wk_start: date):
    ...
    # Optional: BGS Triage Admin early (08:00–10:00) if capacity allows.
    # Only assigns if a suitable BGS triage-capable staff member can cover the full band without clashing
    # with already-fixed work (Front Desk/Triage) or other existing assignments.
    for d in dates:
        role = "Triage_Admin_BGS"
        bs, be = time(8, 0), time(10, 0)
        cands = [s.name for s in tpl.staff if s.can_triage and s.home == "BGS"]
        chosen = pick_for_band(cands, d, "Triage", bs, be)
        if chosen:
            for tt in slots:
                if tt < bs or tt >= be:
...
# Awaiting/PSA Admin is allocated as a stable daily block (see start of day).
            ...
            # --- Proportional weekly smoothing for EMIS/Docman (and keep SLGP mostly on Bookings)
            # Compute an approximate per-day quota from remaining weekly need and remaining days
            # so minutes spread naturally across the week instead of clustering midweek.
            def _day_total(task_key: str) -> int:
                return sum(v for (dd, _nm, tk), v in mins_task_day.items() if dd == d and tk == task_key)

            def _remaining_days_inclusive() -> int:
                return 1 + sum(1 for dd in dates if dd > d)

            def _daily_quota(task_key: str, low_weekly_target: int) -> int:
                if low_weekly_target <= 0:
                    return 0
                done_week = total_mins(task_key)
                remaining = max(0, low_weekly_target - done_week)
                rem_days = max(1, _remaining_days_inclusive())
                return int(math.ceil(remaining / rem_days))

            emis_day_done = _day_total("EMIS")
            doc_day_done  = _day_total("Docman")
            book_day_done = _day_total("Bookings")

            daily_doc_quota  = _daily_quota("Docman", low_doc)
            daily_emis_quota = _daily_quota("EMIS", low_emis)

            filler_tasks: List[str] = []

            # Bookings stays the top filler, but only from 10:30 onwards.
            if target_book > 0 and book_done < low_book and t >= time(10,30):
                filler_tasks.append("Bookings")

            # Docman/EMIS: only push if still below weekly buffer AND today's cap not exceeded.
            # This forces the remaining minutes to spill into later days instead of all landing on Monday.
            if target_doc > 0 and doc_done < low_doc and (daily_doc_quota == 0 or doc_day_done < daily_doc_quota):
                filler_tasks.append("Docman")
            if target_emis > 0 and emis_done < low_emis and (daily_emis_quota == 0 or emis_day_done < daily_emis_quota):
...
# assign fillers to any remaining free staff, pref site logic
            for nm in staff_names:
                ...
                # for EMIS/Docman, keep these mostly on JEN/BGS so SLGP can focus on Bookings.
                # If SLGP is behind Bookings (weekly buffer) and it's after 10:30, prefer Bookings over EMIS/Docman.
                if chosen in ("EMIS","Docman") and staff_by_name[nm].home == "SLGP":
                    # HARD site filter: keep SLGP off EMIS/Docman if any JEN/BGS capacity exists.
                    # If Bookings still below buffer and SLGP staff can do it, convert to Bookings.
                    if t >= time(10,30) and target_book > 0 and book_done < low_book and eligible(nm, "Bookings", d, t, allow_cross_site=True):
                        chosen = "Bookings"
                    else:
                        any_other = False
