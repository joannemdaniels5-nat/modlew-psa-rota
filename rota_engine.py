...
def schedule_week(tpl: TemplateData, week_start: date):
    ...
    for d in dates:
        ...
        for nm in free_staff:
            ...
            # Choose which target is more behind (ratio). Equal priority -> ratio decides; exact tie -> spread.
            doc_ratio  = (doc_rem / target_doc) if target_doc else 0.0
            emis_ratio = (emis_rem / target_emis) if target_emis else 0.0

            preferred = "Docman" if doc_ratio > emis_ratio else "EMIS"
            if abs(doc_ratio - emis_ratio) < 1e-9:
                preferred = "Docman" if (hash(nm) % 2 == 0) else "EMIS"

            other = "EMIS" if preferred == "Docman" else "Docman"

            # Only allow Misc if they cannot do either outstanding task
            if preferred == "Docman" and doc_rem <= 0:
                preferred = other
                other = "Docman"
            if preferred == "EMIS" and emis_rem <= 0:
                preferred = other
                other = "EMIS"

            if eligible(nm, preferred, d, t, allow_cross_site=True):
                assign_block(nm, preferred, d, idx)
            elif eligible(nm, other, d, t, allow_cross_site=True):
                assign_block(nm, other, d, idx)
            else:
                assign_block(nm, "Misc_Tasks", d, idx)
