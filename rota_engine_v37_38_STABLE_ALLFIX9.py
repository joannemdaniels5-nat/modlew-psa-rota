        ...
        for st in staff_list:
            ...
            for bt in cand_times:
                if bt < stt or add_minutes(bt, 30) > end:
                    continue
                if not t_in_range(bt, bw_start, bw_end):
                    continue
                # avoid if fixed assigned at that slot
                if (d, bt, st.name) in fixed_assignments:
                    continue

                # avoid tiny fragments (hard rule: never put break in first hour)
...
def schedule_week(tpl: TemplateData, wk_start: date):
    ...
    for d in dates:
        for site in ("SLGP","JEN"):
            ...
            cands = [s.name for s in tpl.staff if s.can_triage]
    ...
    # Triage (BGS optional 08:00-10:00) — equal priority when capacity allows
    for d in dates:
        site = "BGS"
        role = "Triage_Admin_BGS"
        cands = [s.name for s in tpl.staff if s.can_triage]
        for bs, be in BGS_TRIAGE_BANDS:
            chosen = pick_for_band(cands, d, "Triage", bs, be)
            if not chosen:
                gaps.append((d, bs, role, "Optional BGS triage not assigned (no capacity)"))
                continue
            for tt in slots:
                if tt < bs or tt >= be:
                    continue
...
def build_workbook(tpl: TemplateData, start_monday: date, weeks: int) -> Workbook:
    ...
    for w in range(weeks):
        ...
        # (Disabled conditional formatting: Excel XML repair risk)
...
def _apply_grid_formatting(ws, *, row_height=35.0):
    """Apply consistent row height, full borders, and thick day separators based on Time column."""
    ...
    for r in range(1, ws.max_row + 1):
        ...
        for c in range(1, ws.max_column + 1):
            ws.cell(r, c).border = Border(
                ...
                top=THICK if top_day else THIN,
                bottom=THICK if bottom_day else THIN,
            )
...
def recalc_workbook_from_site_timelines(xlsx_bytes: bytes) -> bytes:
    ...
    for w in week_nums:
        ...
        try:
            _write_targets_heatmap_sheet(wb, w)
        except Exception:
            pass
...
def schedule_week_v37_9(tpl: TemplateData, wk_start: date):
    ...
    # ----------------------------
    # Triage Admin (BGS) optional — schedule if capacity allows
    # ----------------------------
    for d in dates:
        role = "Triage_Admin_BGS"
        cands = [s.name for s in tpl.staff if s.can_triage]
        for bs, be in BGS_TRIAGE_BANDS:
            chosen = pick_for_band(cands, d, "Triage", bs, be)
            if not chosen:
                # optional; do not block build if not possible
                continue
            for tt in slots:
                if tt < bs or tt >= be:
                    continue
