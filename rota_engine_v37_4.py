...
def schedule_week(tpl: TemplateData, wk_start: date):
    ...
    for d in dates:
        ...
        for idx, t in enumerate(slots):
            ...
            # -----------------------------------------------------------------
            # -----------------------------------------------------------------
            # Mandatory: Front Desk — set handover times, min 2.5h blocks (5 slots)
            # Highest priority: assigned BEFORE anything else and locked via fixed_slots.
            #
            # Rule:
            # - Exactly 1 FrontDesk per site per slot.
            # - Avoid 30min/1h fragments: minimum run length is 5 slots (2.5h),
            #   except where the day ends (then use remaining slots).
            # - Handovers should happen at band boundaries with 30min flex.
            #   (i.e. allow change only at these times unless forced by ineligibility)
            # -----------------------------------------------------------------
            FD_MIN_SLOTS = 5  # 2.5h
            ...
            def _is_fd_handover(tt: time) -> bool:
                return tt in FD_HANDOVER

            def _fd_eligible_for_slot(nm: str, site: str, dd: date, tt: time) -> bool:
                st = staff_by_name[nm]
                if st.home != site:
                    return False
            ...
            def _fd_can_cover_block(nm: str, site: str, dd: date, start_idx: int, need_slots: int) -> bool:
                # Need continuous eligibility across the whole block.
                for j in range(start_idx, min(start_idx + need_slots, len(slots))):
                    tt = slots[j]
                    if not _fd_eligible_for_slot(nm, site, dd, tt):
                        return False
                    # Must not already be locked to another fixed assignment
                    if (dd, tt, nm) in fixed_slots:
                        return False
                return True

            def _pick_fd_staff(site: str, dd: date, start_idx: int, need_slots: int) -> Optional[str]:
                cands = []
                for nm in staff_names:
                    if not _fd_can_cover_block(nm, site, dd, start_idx, need_slots):
                        continue
                    cands.append(nm)
                if not cands:
                    return None
            ...
            for site in SITES:
                ...
                # If someone is already assigned FD in this slot, keep the best one (dedupe).
                ...
                if len(current) == 1:
                    nm = current[0]
                    fd_last[site] = nm
                    fd_run[site] = fd_run.get(site, 0) + 1
                    fixed_slots.add((d, t, nm))
                    continue

                # Decide whether to keep the previous FD person
                prev = fd_last.get(site)
                prev_ok = bool(prev and _fd_eligible_for_slot(prev, site, d, t) and is_free(prev, d, t))

                remaining_day_slots = len(slots) - idx
                need_slots = min(FD_MIN_SLOTS, remaining_day_slots)

                # Keep prev if:
                # - still eligible
                # - and we're mid-run (<min) OR it isn't a handover time
                # - and they can actually cover the next block length if we're at/near handover
                if prev_ok:
                    if fd_run.get(site, 0) < need_slots and not _is_fd_handover(t):
                        a[(d, t, prev)] = role
                        fixed_slots.add((d, t, prev))
                        add_mins(d, prev, "FrontDesk", SLOT_MIN)
                        fd_last[site] = prev
                        fd_run[site] = fd_run.get(site, 0) + 1
                        continue

                    if (not _is_fd_handover(t)):
                        # Not a handover time — keep for stability.
                        a[(d, t, prev)] = role
                        fixed_slots.add((d, t, prev))
                        add_mins(d, prev, "FrontDesk", SLOT_MIN)
                        fd_last[site] = prev
                    ...
                    # At a handover time: only switch if prev can't cover a full next block
                    if _fd_can_cover_block(prev, site, d, idx, need_slots):
                        a[(d, t, prev)] = role
                        fixed_slots.add((d, t, prev))
                        add_mins(d, prev, "FrontDesk", SLOT_MIN)
                        fd_last[site] = prev
                ...
                # Need to pick (or switch) FD staff now
                chosen = _pick_fd_staff(site, d, idx, need_slots)
                if not chosen:
                    gaps.append((d, t, role, f"No suitable staff for FD block (need {need_slots} slots)"))
                    fd_last[site] = None
                    fd_run[site] = 0
                else:
                    a[(d, t, chosen)] = role
                    ...
                    # reset run counter for new person
                    fd_last[site] = chosen
                    fd_run[site] = 1
            ...
            if t == time(10,30):
                ...
                window_slots = [tt for tt in slots if t_in_range(tt, time(10,30), time(16,0))]

                def email_eligible(nm: str, tt: time) -> bool:
                    st = staff_by_name[nm]
                    if not st.can_email:
                        return False
                    if holiday_kind(nm, d, tpl.hols):
                ...
                for tt in window_slots:
                    # already covered (by earlier allocation)
                    cur = [nm for nm in staff_names if a.get((d, tt, nm)) == "Email_Box"]
                    if cur:
                        last_nm = cur[0]
                        continue

                    # prefer staying with the same person if possible
                    chosen = None
                    if last_nm and email_eligible(last_nm, tt) and is_free(last_nm, d, tt):
                        chosen = last_nm
                    else:
                        # 1) prefer site-of-day first
                        cands = [nm for nm in staff_names if staff_by_name[nm].home == pref_site and is_free(nm, d, tt) and email_eligible(nm, tt)]
                        # 2) fallback cross-site
                        if not cands:
                            cands = [nm for nm in staff_names if is_free(nm, d, tt) and email_eligible(nm, tt)]
                        if cands:
                            cands.sort(key=email_score)
                            chosen = cands[0]

                    if chosen:
                        # lock chosen for this slot (email should not be stolen later)
                        stop_block(chosen, d)
                        ...
                        gaps.append((d, tt, "Email_Box", "No suitable staff for Email slot (10:30–16:00)"))

            # Safety de-dup per slot (never allow >1 Email)
            if t_in_range(t, time(10,30), time(16,0)):
                current = [nm for nm in staff_names if a.get((d, t, nm)) == "Email_Box"]
                if len(current) > 1:
                    def email_score_slot(nm: str):
                        st = staff_by_name[nm]
                        pref = 0 if st.home == email_site_for_day(d) else 1
                        w = task_weight(st, "Email")
                        used = mins_task.get((nm, "Email"), 0)
                        return (pref, -w, used, nm.lower())
                    current.sort(key=email_score_slot)
                    keep = current[0]
                    for extra in current[1:]:
                        del a[(d, t, extra)]
