...
def schedule_week(tpl: TemplateData, week_start: date) -> Tuple[Dict[Tuple[date, time, str], str], Dict[Tuple[date, time], Set[str]], List[Tuple[date, time, str, str]], List[date], List[time], Dict[str, Dict[str, Optional[time]]]]:
    ...
    def fill_filler(d: date, idx: int):
        t = slots[idx]
        # soft quotas across week: prefer task still behind target
        emis_target = float(tpl.weekly_targets.get("EMIS", 0.0) or 0.0) * 60
        doc_target = float(tpl.weekly_targets.get("Docman", 0.0) or 0.0) * 60
        ...
        prefer = "EMIS" if (emis_target > 0 and emis_done < emis_target) else ("Docman" if (doc_target > 0 and doc_done < doc_target) else "EMIS")
        ...
        for nm, st in staff_by_name.items():
            ...
            chosen = None
            # After 16:00, allow bookings/email/awaiting to drop if phones/frontdesk require; filler only if free anyway
            if prefer == "EMIS" and can_do(nm, "EMIS", d, t):
                chosen = "EMIS"
            elif prefer == "Docman" and can_do(nm, "Docman", d, t):
                chosen = "Docman"
            else:
                if can_do(nm, "Docman", d, t):
                    chosen = "Docman"
                elif can_do(nm, "EMIS", d, t):
                    chosen = "EMIS"
                else:
                    chosen = "Unassigned"
